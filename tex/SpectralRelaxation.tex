...
\begin{document}
  ...
  \begin{abstract}
    ...
    We further identify a two-regime growth law for the projectable mode count ---
    polynomial in the BFS shell regime, exponential in the trajectory-branching
    regime --- as a structural candidate for a cosmological model of decelerated
    then accelerated expansion, compatible with recent DESI evidence for
  ...
  \section{Stratigraphic Amplification: Numerical Evaluation}
    ...
    \subsection{Ordering of the predicted hierarchy}
      ...
      \begin{figure}[h]
        \centering
        \includegraphics[width=0.90\textwidth]{fig3_reading_A_ratios}
  ...
  \rho_{\mathrm{KM}}(2 - \lambda)$ of the Kesten--McKay density combined with
    ...
    \subsection{Open directions}
      ...
      \begin{figure}[h]
        \centering
        \includegraphics[width=0.72\textwidth]{fig5_O1_support}
  ...
  \mathcal{M}_2$, which for the physically relevant assignment
      ...
      \medskip
      \noindent\textbf{(O4) Cosmological expansion and the two growth regimes.}
      ...
      \begin{itemize}
        ...
        \item an \emph{exponential regime}, in which the number of distinct admissible
        trajectories of length $n$ grows as a branching process, governing the late
        cascade once the spectral position is sufficiently displaced from the origin.
      \end{itemize}
      If the projectable volume $N_{\mathrm{proj}}(n)$ is interpreted as the
      effective ``size'' of the observable universe at cascade rank $n$, these two
      regimes provide a structural two-phase model for cosmic expansion:
      decelerated growth in the polynomial regime, transitioning to accelerated growth
      in the exponential (trajectory-branching) regime.
      ...
      evolves from below $-1$ (phantom-like, in the early exponential-branching
      phase) toward $w > -1$ (quintessence-like, as the branching rate slows when
      the admissible neighbourhood $\mathcal{A}(O_n)$ becomes progressively
      constrained by accumulated projective history).
      The formalisation of this cosmological model is deferred to the Cosmology
      companion paper.
  ...
  \section{Conclusion}
    ...
    $c_\chi \to \delta_{\mathrm{pair}} \to \beta^*$~\cite{Beau2026a28,Beau2026a29};
