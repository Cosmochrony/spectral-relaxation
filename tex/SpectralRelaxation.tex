...
cascade rank?}
  ...
  \subsection{Position within the Cosmochrony programme}
    ...
    This paper explores a specific model of the projective cascade --- LPS Ramanujan
    graphs with fixed prime~$p$ --- and establishes a structural negative result:
    the admissibility threshold $\Lambda_{\mathrm{proj}}$ is asymptotically static
    in this model, which is insufficient to produce the observed inter-generation
    mass hierarchy.

    The derivation of the mass hierarchy within the Cosmochrony programme rests on
    a distinct mechanism established in the O-series: the transfer chain
    $c_\chi \to \delta_{\mathrm{pair}} \to \beta^*$, proved unconditionally
    in~\cite{Beau2026a28} and validated numerically across all conjugate pairs
    in~\cite{Beau2026a29}.
    That chain operates within a fixed Heisenberg Cayley graph
    $\mathrm{Heis}_3(\mathbb{Z}/q\mathbb{Z})$ via the decay of the pair capacity
    $\sigma_{\mathrm{pair}}(n)$ along the BFS cascade, and does not depend on the
    LPS model studied here.
    ...
    The development of that cosmological model is left to future work.
...
\mathcal{M}_2$, which for the physically relevant assignment
  ...
  the present results identify the structural obstacle and the open direction that a
  complete cosmological model must address.
